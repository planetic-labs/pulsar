import pytest
from app.file_dedupe import canonicalize_title, has_gallery_marker, choose_preferred_file, dedupe_gallery_variants
...
def test_choose_preferred_file():
    f_reg = DriveFile(file_id="1", name="Video.mp4", mime_type="video/mp4", size="100", created_time="2024-01-01", parents=())
    f_gal = DriveFile(file_id="2", name="Video.Gallery.mp4", mime_type="video/mp4", size="100", created_time="2024-01-01", parents=())
    ...
    f_larger = DriveFile(file_id="3", name="Video.mp4", mime_type="video/mp4", size="200", created_time="2024-01-01", parents=())
    assert choose_preferred_file(f_reg, f_larger) == f_larger
    
    # Prefer newer if same type and size
    f_newer = DriveFile(file_id="4", name="Video.mp4", mime_type="video/mp4", size="100", created_time="2023-01-01", parents=())
    ...
    # Let's re-read: `if candidate_created and (not current_created or candidate_created < current_created): return candidate`
