...
- **Stage 3 (Indexing)**: Generates embeddings for text chunks using the Infinity Remote Service and upserts them into Manticore.
...
- **Manticore Search**: A high-performance database used for semantic and lexical search. Each record in Manticore corresponds to a text chunk in SQLite.
...
2.  **Lexical Retrieval**: Uses Manticore's built-in fulltext search `MATCH()` for precise "keyword-based" search.
...
3.  **Search**: When a user queries, the system fetches embeddings (or pulls from cache), queries Manticore, hydrates results with fresh metadata from SQLite, and applies highlighting.
