    ...
    async def process_task(self, task_id: int, task_type: str, payload: dict):
        async with self.semaphore:
            ...
            with db_connection(settings) as conn:
                conn.execute("UPDATE tasks SET status = 'running', updated_at = CURRENT_TIMESTAMP WHERE id = ?", (task_id,))
            ...
            try:
                if task_type == "ingest_video":
                    ...
                    result = await loop.run_in_executor(
                        None, 
                        lambda: ingest_drive_file(file_id, diarize=diarize, download_progress_callback=progress_callback)
                    )
